function  buildFFTmaskCW(N, Fs, loCut, fftsize)
%function buildFFTmaskCW(N, Fs, loCut, fftsize)
%            
% TODO This was the code for the SSB generator. I changed some numbers for 
% a quick CW test. 
%                                                             
...
fnameR = sprintf( '%s\\FFTmaskCW_R.txt', CurrentPath);
fnameI = sprintf( '%s\\FFTmaskCW_I.txt', CurrentPath);
fidR=fopen(fnameR, 'w');
fidI=fopen(fnameI, 'w');

for iter=1:1:NumFilt
    hiCut=BW(iter);
msg = sprintf('Costruzione del filtro CW da %d Hz', hiCut);
disp(msg);

transwidth = 300;            % larghezza della zona di transizione
flp = (hiCut - loCut)/2 - 40;     % larghezza del prototipo low pass
flpst = flp + transwidth;    % inizio stop band per il LP prototipo
fshift = flp + loCut + 40 + 600;   % ammontare dello shift verso destra
...
if(fidR > 0)
    if(iter == 1)
      fprintf(fidR, 'const float FFTmaskCW_R[%d][1024] =\n{\n', NumFilt);
    end
    
    fprintf(fidR, '// CW_R PB width %d Hz\n{\n', BW(iter));
    fprintf(fidR, '%13.10ff, %13.10ff, %13.10ff, %13.10ff, %13.10ff, %13.10ff, %13.10ff, %13.10ff, \n', realf(iter,:));
    if(iter == NumFilt) fprintf(fidR, '}\n};\n');
    else  fprintf(fidR, '},\n');
    end    
else
    disp('Errore aprendo il file FFTmaskCW_R.txt');
end
end
fclose(fidR);
...
if(fidI > 0)
    if(iter == 1)
      fprintf(fidI, 'const float FFTmaskCW_I[%d][1024] =\n{\n', NumFilt);
    end  
    fprintf(fidI, '// CW_I PB width %d Hz\n{\n', BW(iter));
    fprintf(fidI, '%13.10ff, %13.10ff, %13.10ff, %13.10ff, %13.10ff, %13.10ff, %13.10ff, %13.10ff, \n', imagf(iter,:));
    if(iter == NumFilt) fprintf(fidI, '}\n};\n');
    else  fprintf(fidI, '},\n');
    end    
else
    disp('Errore aprendo il file FFTmaskCW_I.txt');
end    
end
fclose(fidI);
end
% [EOF]
# Created by Octave 6.3.0, Sat Jan 22 21:58:40 2022 GMT <unknown@DESKTOP-LI6GKLP>
